...
const generateAccessTokenAndRefreshToken = async (userId) => {
  try {
    ...
    // to stop mongoose property kick in we use validateBeforeSave: false
...
const registerUser = asyncHandler(async (req, res) => {
  ...
  // check if the  user with this email or username is already present in the database
  if (
  ...
  // getting  the existing user data from the DB
...
const getWatchHistory = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, user[0].watchHistory, "Fetched history successfully")
    );
});
